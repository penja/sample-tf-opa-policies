...
#
# To enforce this the policy must check that all uses of KMS keys are referenced from a data source
# and that the data source itself uses an allowed key name.
#
# KMS is used in many AWS services. This policy attempts to deal with all cases:
# CloudTrail, CloudWatch, DynamoDB, EBS, Elastic Transcoder, EMR, Redshift, RDS,
# Secrets Manager, SES, S3 and SSM Parameter Store.
...
allowed_kms_keys := ["pg-kms-key", "alias-1"]
...
kms_eval_expression(plan, expr) := constant_value if {
...
kms_eval_key_name(plan, res) := constant_value if {
...
deny contains reason if {
	...
	r.mode == "data"
	r.type == "aws_kms_key"
	key_alias := kms_eval_key_name(tfplan, r)
	key_name := trim_prefix(key_alias, "alias/")
	not key_name in allowed_kms_keys

	reason := sprintf(
		"%-40s :: KMS key name '%s' not in permitted list",
		[concat(".", [r.type, r.name]), key_name],
	)
}
...
deny contains reason if {
	...
	kms_key := kms_eval_expression(tfplan, r.expressions.replication_configuration[_].rules[_].destination[_].replica_kms_key_id)
	not startswith(kms_key, "data.aws_kms_key.")

	reason := sprintf(
		"%-40s :: replication KMS Master key ID '%s' not derived from data source!",
		[concat(".", [r.type, r.name]), kms_key],
	)
}
...
deny contains reason if {
	...
	r.mode == "managed"
	r.type == "aws_s3_bucket"
	kms_key := kms_eval_expression(tfplan, r.expressions.server_side_encryption_configuration[_].rule[_].apply_server_side_encryption_by_default[_].kms_master_key_id)
	not startswith(kms_key, "data.aws_kms_key.")

	reason := sprintf(
		"%-40s :: server_side_encryption KMS Master key ID '%s' not derived from data source!",
		[concat(".", [r.type, r.name]), kms_key],
	)
}
...
deny contains reason if {
	tfrun.is_destroy == false
	r := tfplan.configuration.root_module.resources[_]
	a := kms_attributes[r.type][_]
	r.mode == "managed"
	kms_key := kms_eval_expression(tfplan, r.expressions[a])
	not startswith(kms_key, "data.aws_kms_key.")

	reason := sprintf(
		"%-40s :: KMS Key not derived from data source (%s=%s) :: ",
...
deny contains reason if {
	...
	config := kms_eval_expression(tfplan, r.expressions.configuration)
	...
	reason := sprintf(
		"%-40s :: configuration disc encryption key '%s' not from permitted list",
		[concat(".", [r.type, r.name]), key_name],
	)
}
...
deny contains reason if {
	...
	kms_key := kms_eval_expression(tfplan, r.expressions.s3_action.kms_key_arn)
	not startswith(kms_key, "data.aws_kms_key.")

	reason := sprintf(
		"%-40s :: s3_action KMS Master key ID '%s' not derived from data source!",
		[concat(".", [r.type, r.name]), kms_key],
	)
}
...
deny contains reason if {
	...
	kms_key := kms_eval_expression(tfplan, r.expressions.s3_destination.kms_key_arn)
	not startswith(kms_key, "data.aws_kms_key.")

	reason := sprintf(
		"%-40s :: s3_destination KMS Master key ID '%s' not derived from data source!",
		[concat(".", [r.type, r.name]), kms_key],
	)
}
